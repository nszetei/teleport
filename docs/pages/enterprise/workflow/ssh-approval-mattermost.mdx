...
<TabItem label="Helm">
  ...
  See an example for `teleport-mattermost-helm.yaml` under Helm in the next section!
</TabItem>
</Tabs>
...
<TabItem scopes={["oss", "enterprise", "cloud"]} label="Helm values">
```yaml
(!examples/resources/plugins/teleport-mattermost-helm.yaml!)
```
...
```code
kubectl create secret generic teleport-slack-identity --from-file=auth_id=identity
```
</TabItem>
</Tabs>
